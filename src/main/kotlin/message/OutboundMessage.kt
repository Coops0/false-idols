...
class AssignRoleOutboundMessage(
    ...
    val teammates: Array<PlayerName>? = null,
    val satan: PlayerName? = null
...
class RequestActionChoiceOutboundMessage(
    ...
    data class ActionSupplementedPlayer(
        val name: PlayerName,
...
class InvestigationResultOutboundMessage(
    val target: PlayerName,
    val simpleRole: SimpleRole,
) : OutboundMessage("investigation_result") {
    constructor(target: PlayerName, complexRole: ComplexRole) : this(target, SimpleRole.fromComplexRole(complexRole))
}
